...
import { createMessageShell } from "./messageShell.ts";
import { hideEmptyState } from "./emptyState.ts";
...
export function appendMessage(sender: string, content: string, type: string, skipHistory = false) {
  ...
  const { messageElement, contentElement } = createMessageShell(sender, type);
