 ...
 *   never applied — an export never contains them, and a hand-edited file must
 *   not be able to swap the user's API keys or server URLs.
 */
...
/** Longest memory `addMemory` keeps; anything past this is truncated on write. */
const MEMORY_TEXT_LIMIT = 600;

/** Credential keys, which an import must never write. */
const CREDENTIAL_KEYS: string[] = [
  STORAGE_KEYS.lmStudioServerUrl,
  STORAGE_KEYS.ollamaServerUrl,
];

/** Prefixes of the per-service credential keys, which an import must never write. */
const CREDENTIAL_KEY_PREFIXES: string[] = [
...
/**
 * Normalizes memory text exactly the way `addMemory` does before storing it, so
 * a memory that was truncated on the way in still matches its longer original
 * when the same file is imported again.
 */
const normalizeMemory = (text: string): string => text.trim().slice(0, MEMORY_TEXT_LIMIT);

const isCredentialKey = (key: string): boolean =>
  CREDENTIAL_KEYS.includes(key) || CREDENTIAL_KEY_PREFIXES.some((prefix) => key.startsWith(prefix));
...
function mergeMemories(bundle: ImportBundle, summary: ImportSummary) {
  ...
  const known = new Set(getMemories().map(normalizeMemory));
...
function mergeSettings(bundle: ImportBundle, options: ImportOptions, summary: ImportSummary) {
  ...
  for (const [key, value] of Object.entries(bundle.settings)) {
    if (typeof key !== "string" || typeof value !== "string" || isCredentialKey(key)) {
      continue;
    }
    try {
