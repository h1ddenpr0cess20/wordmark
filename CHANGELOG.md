...
### Added
- **OpenRouter provider** — adds OpenRouter as a fifth AI service alongside OpenAI and xAI, using its OpenAI-compatible Responses API (`https://openrouter.ai/api/v1/responses`). Configure it with its own API key in Settings → API Keys; its model list is fetched live from OpenRouter's combined multi-vendor catalog, defaulting to `nvidia/nemotron-3-ultra-550b-a55b:free`. Document attachments are extracted client-side (like the local providers) since OpenRouter has no OpenAI-compatible vector-store/file API, and requests set `store: false` and omit `previous_response_id` since OpenRouter's Responses API rejects server-side state storage. Since OpenRouter also exposes embedding models — listed separately from its chat catalog, via `/embeddings/models` rather than `/models` — attached documents get full local RAG (chunking, embedding, hybrid dense/BM25 retrieval) the same way LM Studio/Ollama do, rather than just being extracted to plain text. The embedding-model dropdown in Settings → API Keys is enabled for it, defaulting to `nvidia/nemotron-3-embed-1b:free` when the account's fetched embedding-model list actually contains it, and otherwise falling back to whatever the account does have.
