...
- Wordmark is a client-side AI assistant with OpenAI, LM Studio, and Ollama support, optional tool calling, TTS, themes, and local-first persistence.
...
- Provider flexibility: hosted OpenAI Responses API, plus local LM Studio or Ollama servers
- Tool calling framework with built-in weather, provider web search, Code Interpreter, optional image generation/file search, plus your own MCP servers
...
- Themes and responsive UI
 - Header shortcuts and indicators:
   ...
   - Hover the model name to see a provider tooltip (OpenAI, LM Studio, or Ollama).
