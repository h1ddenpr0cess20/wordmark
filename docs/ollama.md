...
4. Set the base URL to `http://localhost:11434` and click “Save Ollama Settings”.
5. Open Settings → Model and click “Refresh Local Models”.
6. Select your Ollama model and start chatting.

Notes
- Ollama’s local server does not require an API key by default. If yours does — an `OLLAMA_API_KEY`, or a reverse proxy in front of it — enter it in the optional **Ollama API Key** field in the same settings block; it is sent as a bearer token on model, chat, and embedding requests. Leave it blank otherwise — a blank key means no `Authorization` header is sent at all.
