...
- `file_search` (type: `builtin`) — Vector store search across documents you have added to a store (OpenAI only). Rendered in reasoning timeline. Attaching a file to a message no longer goes through it: OpenAI and xAI both take the file directly as an `input_file` reference.
