...
- **xAI**: 26 voices — the original 5 (male: leo, rex, sal; female: ara, eve) plus the 21 flagship voices xAI shipped on 2026-07-06 (Altair, Atlas, Carina, Castor, Celeste, Cosmo, Helios, Helix, Iris, Kepler, Lumen, Luna, Lux, Naksh, Orion, Perseus, Rigel, Sirius, Ursa, Zagan, Zenith). xAI's `/v1/tts/voices` endpoint does not classify built-in voices by gender, so the flagship voices are grouped under an **Other** category in the selector. Uses the xAI TTS API at `api.x.ai/v1/tts` with automatic language detection. Voice instructions are not supported.
