...
- **Tool calling** — built-in weather, provider web + X search, Code Interpreter, image generation, optional file search over vector stores (OpenAI), and custom MCP servers ([Tool Calling](docs/tool-calling.md))
- **Documents** — attach files or folders in any text/code format plus PDF, Office, OpenDocument, and ebooks; hosted providers take the file itself, local providers extract and search it in-browser with embeddings so nothing leaves your machine ([Documents & Attachments](docs/documents.md))
...
- **Prompt queuing** — type while a response is still streaming and the message reaches the assistant mid-answer: the reply in flight is interrupted and resumed with your message added, keeping every word already written ([UI and UX](docs/ui-and-ux.md#prompt-queuing))
